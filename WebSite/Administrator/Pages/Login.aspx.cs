        ...
        if (IsValid)
        {
            ...
            if (dt.Rows.Count > 0)
            {
                if (Request.Browser.Cookies)
                {
                    ...
                    aCookie.Values.Add("StaffID", row["StaffID"].ToString());
                    aCookie.Values.Add("StaffName", Server.UrlEncode(row["StaffName"].ToString()));
                    aCookie.Values.Add("Place_ID", row["Place_ID"].ToString());
