...
public partial class Administrator_Pages_MemberCard_ReportServiceTransaction : System.Web.UI.Page
{
    protected void Page_Load(object sender, EventArgs e)
    {
        ((Administrator_MasterPage_MasterPageMemberCard)Master).OptionRule = DTO.Permission.EnumOption.RPTServiceTransaction;
    ...
    private void LoadData()
    {
        ...
        List<DTO.tblMemberCardServiceTransaction> lsArray = BUS.tblMemberCardServiceTransaction.GetByServiceDate(s, e, Place_ID);
        ...
        //string str = "";
        //List<string> ls = lsArray.GroupBy(gb => gb.PlaceName).Select(se => se.FirstOrDefault().PlaceName).ToList();
        //foreach (string s1 in ls)
        //{
        //    if (str != "")
        //        str += "    " + "     |     " + s1 + ":   " + lsArray.Where(w => w.PlaceName == s1).Count().ToString("N0");
        //    else
        //        str += s1 + ": " + lsArray.Where(w => w.PlaceName == s1).Count().ToString("N0");
        //}
        //lDanhSach.Text = str;
    ...
    protected void btnExport_Click(object sender, EventArgs e)
    {
        ...
        System.Data.DataTable dt = BUS.tblMemberCardServiceTransaction.GetDataByServiceDate(sta, end, Place_ID);
        ...
        dt.Columns["Increase_Decrease"].ColumnName = "Loại giao dịch";
        dt.Columns["Description"].ColumnName = "Cách tính điểm";
        dt.Columns["PointRec"].ColumnName = "Điểm tích lũy/trả thưởng";
        dt.Columns["ServiceAmount"].ColumnName = "Trị giá";
        dt.TableName = "Report_Service_Transaction";
        ds.Tables.Add(dt);
        if (!System.IO.Directory.Exists(Server.MapPath("\\Output")))
            System.IO.Directory.CreateDirectory(Server.MapPath("\\Output"));
        string Path = Server.MapPath(String.Format("\\Output\\{0}_{1}.xlsx", "Report_Service_Transaction", DateTime.Now.ToString("yyyy_MM_dd_HH_mm_ss")));
